...
def main():
    ...
    if linkedin_token:
        try:
            ...
            logger.error("LinkedIn failed: " + str(exc))
    else:
        linkedin_error = "LINKEDIN_ACCESS_TOKEN not set."
    ...
    job_list = "\n".join(job_lines)
    if linkedin_success:
        subject = "Walmart Bot - LinkedIn posted (" + str(len(jobs)) + " jobs)"
        body = "Post published!\n\n" + post_text + "\n\nJobs:\n" + job_list
    else:
        subject = "Walmart Bot - Draft ready (" + str(len(jobs)) + " jobs)"
        body = ("Reason not posted: " + linkedin_error + "\n\nDRAFT:\n" + post_text
